 ...
 */
public class Exercise2_FenwickTree2DPURQ {

    static class FenwickTree2DPURQ {
        private final long[][] fenwickTree;
        final int rows;
        final int columns;

        public FenwickTree2DPURQ(int rows, int columns) {
            this.rows = rows;
        ...
        // O(log(m) * log(n))
        public int rangeQuery(int row1, int column1, int row2, int column2) {
    ...
    public static void main(String[] args) {
        ...
        FenwickTree2DPURQ fenwickTree2DPURQ = new FenwickTree2DPURQ(values);
        System.out.println("Fenwick Tree");
        printFenwickTree(fenwickTree2DPURQ);
        System.out.println("Range query: " + fenwickTree2DPURQ.rangeQuery(1, 2, 3, 4)
                + " expected: 12");
    ...
    private static void printFenwickTree(FenwickTree2DPURQ fenwickTree2DPURQ) {
        for (int row = 1; row <= fenwickTree2DPURQ.rows; row++) {
            for (int column = 1; column <= fenwickTree2DPURQ.columns; column++) {
                 System.out.print(fenwickTree2DPURQ.rangeQuery(row, column, row, column));
