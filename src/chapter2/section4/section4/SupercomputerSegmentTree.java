 ...
 */
public class SupercomputerSegmentTree {

    public static void main(String[] args) throws IOException {
        FastReader.init();
        OutputWriter outputWriter = new OutputWriter(System.out);
        int bitsInMemory = FastReader.nextInt();
        int[] bits = new int[bitsInMemory + 1];
        int queries = FastReader.nextInt();

        SegmentTree segmentTree = new SegmentTree(bits);

        for (int q = 0; q < queries; q++) {
            String queryType = FastReader.next();
            if (queryType.equals("F")) {
                int bitIndex = FastReader.nextInt();
                long bitValue = segmentTree.rsq(bitIndex, bitIndex);
                int newBitValue = bitValue == 0 ? 1 : 0;
                segmentTree.update(bitIndex, bitIndex,newBitValue);
            } else {
                int left = FastReader.nextInt();
                int right = FastReader.nextInt();
                long bitsSet = segmentTree.rsq(left, right);
