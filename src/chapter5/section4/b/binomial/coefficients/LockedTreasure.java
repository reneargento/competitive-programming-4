...
public class LockedTreasure {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < tests; t++) {
            int bandits = FastReader.nextInt();
            int groupSize = FastReader.nextInt();

            long minimumLocks = binomialCoefficient(bandits, groupSize);
            outputWriter.printLine(minimumLocks);
        }
        outputWriter.flush();
    }
    ...
    private static long binomialCoefficient(int totalNumbers, int numbersToChoose) {
        ...
        for (int i = 0; i < numbersToChoose - 1; i++) {
