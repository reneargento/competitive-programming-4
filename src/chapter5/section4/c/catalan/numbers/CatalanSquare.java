...
public class CatalanSquare {
    ...
    public static void main(String[] args) throws IOException {
        ...
        BigInteger squareCatalanN = computeSquareCatalanN(n);
        outputWriter.printLine(squareCatalanN);
        outputWriter.flush();
    }

    private static BigInteger computeSquareCatalanN(int n) {
        BigInteger squareCatalanN = BigInteger.ZERO;
        BigInteger[] catalanNumbers = catalanNumbers();

        for (int i = 0; i <= n; i++) {
            BigInteger term = catalanNumbers[i].multiply(catalanNumbers[n - i]);
            squareCatalanN = squareCatalanN.add(term);
        }
        return squareCatalanN;
    }
