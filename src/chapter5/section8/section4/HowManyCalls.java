...
public class HowManyCalls {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (n != 0 || base != 0) {
            long callsDigit = computeCallsDigit(n, base);
            outputWriter.printLine(String.format("Case %d: %d %d %d", caseId, n, base, callsDigit));
    ...
    private static long computeCallsDigit(long n, int base) {
        long fibonacci = computeFibonacci(n, base);
        if (fibonacci == 0) {
            fibonacci = base;
        }
        long calls = 2 * fibonacci - 1;
        return calls % base;
    }

    private static long computeFibonacci(long number, long mod) {
