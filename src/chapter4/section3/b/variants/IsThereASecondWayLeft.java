...
public class IsThereASecondWayLeft {
    ...
    private static final long NO_WAY = Long.MAX_VALUE;
    private static final long NO_SECOND_WAY = Long.MAX_VALUE - 1;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= tests; t++) {
            ...
            for (int i = 0; i < connections; i++) {
                ...
                int end = FastReader.nextInt() - 1;
                int cost = FastReader.nextInt();
                Edge edge = new Edge(i, start, end, cost);

                adjacencyList[start].add(edge);
            }

            new SecondBestMST(adjacencyList);
            long secondMSTWeight = SecondBestMST.computeSecondBestMST();
            String result;
            if (secondMSTWeight == NO_WAY) {
                result = "No way";
            } else if (secondMSTWeight == NO_SECOND_WAY) {
                result = "No second way";
            } else {
                result = String.valueOf(secondMSTWeight);
            }
            outputWriter.printLine(String.format("Case #%d : %s", t, result));
        }
        outputWriter.flush();
    }

    private static class SecondBestMST {
        ...
        public static long computeSecondBestMST() {
            ...
            if (secondMSTWeight == Long.MAX_VALUE) {
                return NO_SECOND_WAY;
            }
        ...
        // Computes the highest edge weight on the paths from vertexID1 and vertexID2 to their LCA
        private static int lca(int vertexID1, int vertexID2) {
            int highestWeightsInAncestorPath = -2;
            ...
            for (int i = MAX_PATH - 1; i >= 0; i--) {
                if (distances[vertexID1] - distances[vertexID2] >= (1 << i)) {
                    highestWeightsInAncestorPath =
                            Math.max(highestWeightsInAncestorPath, dpMaxWeights[vertexID1][i]);
            ...
            for (int i = MAX_PATH - 1; i >= 0; i--) {
                if (vertexIDsAbove[vertexID1][i] != -1
                    ...
                    highestWeightsInAncestorPath = Math.max(highestWeightsInAncestorPath, highestWeightDP);
                    vertexID1 = vertexIDsAbove[vertexID1][i];
                    vertexID2 = vertexIDsAbove[vertexID2][i];
                }
            }
            int highestRootWeight = Math.max(dpMaxWeights[vertexID1][0], dpMaxWeights[vertexID2][0]);
            return Math.max(highestWeightsInAncestorPath, highestRootWeight);
        }
    }
