...
/**
 * Created by Rene Argento on 14/09/2026.
 */
public class RepeatedSubstringsKattis {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < tests; t++) {
            ...
            int repeatedSubstrings = countRepeatedSubstrings(string);
            outputWriter.printLine(repeatedSubstrings);
        }
        outputWriter.flush();
    }

    private static int countRepeatedSubstrings(String string) {
        SuffixArrayNlgN suffixArray = new SuffixArrayNlgN(string);
        int repeatedSubstrings = 0;

        for (int i = 2; i < suffixArray.lcp.length; i++) {
            if (suffixArray.lcp[i] > suffixArray.lcp[i - 1]) {
                repeatedSubstrings += suffixArray.lcp[i] - suffixArray.lcp[i - 1];
            }
        }
