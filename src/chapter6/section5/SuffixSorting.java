...
public class SuffixSorting {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (line != null) {
            char terminatingSymbol = (char) 0;
            String string = line + terminatingSymbol;
            SuffixArrayNlgN suffixArray = new SuffixArrayNlgN(string);

            int indexesNumber = FastReader.nextInt();
            for (int i = 0; i < indexesNumber; i++) {
                int index = FastReader.nextInt() + 1;
                int position = suffixArray.suffixArray[index];

                if (i > 0) {
                    outputWriter.print(" ");
                }
                outputWriter.print(position);
            }
