...
public class RepeatedSubstrings {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String lcs = computeLcs(string);
        outputWriter.printLine(lcs);
        outputWriter.flush();
    }

    private static String computeLcs(String string) {
        SuffixArrayNlgN suffixArray = new SuffixArrayNlgN(string);
        int longestCommonSubstringLength = computeLcsLength(suffixArray);
        String longestCommonSubstring = null;

        for (int i = 1; i < suffixArray.lcp.length; i++) {
            int lcp = suffixArray.lcp[i];

            if (lcp == longestCommonSubstringLength) {
                int startSuffixIndex = suffixArray.suffixArray[i];
                String substring = string.substring(startSuffixIndex, startSuffixIndex + lcp);

                if (longestCommonSubstring == null
                        || longestCommonSubstring.compareTo(substring) > 0) {
    ...
    private static int computeLcsLength(SuffixArrayNlgN suffixArray) {
        int longestCommonSubstringLength = 0;
        for (int i = 1; i < suffixArray.lcp.length; i++) {
            longestCommonSubstringLength = Math.max(longestCommonSubstringLength, suffixArray.lcp[i]);
        }
