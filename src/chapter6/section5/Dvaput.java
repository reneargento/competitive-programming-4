...
public class Dvaput {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int longestRepeatingSubstring = computeLcs(message);
        outputWriter.printLine(longestRepeatingSubstring);
        outputWriter.flush();
    }

    private static int computeLcs(String message) {
        int longestRepeatingSubstring = 0;
        SuffixArrayNlgN suffixArray = new SuffixArrayNlgN(message);

        for (int lcp : suffixArray.lcp) {
            longestRepeatingSubstring = Math.max(longestRepeatingSubstring, lcp);
        }
